...
@DATASETS.register_module()
class CustomNuScenesOfflineLocalMapDatasetWithSatelliteMap(CustomNuScenesOfflineLocalMapDataset):
    r"""NuScenes Dataset with Satellite Map.

    This dataset extends the offline local map dataset with satellite map functionality.
    """
    def __init__(self,
                 satellite_map_root=None,
                 *args, 
                 **kwargs):
        super().__init__(*args, **kwargs)
        self.satellite_map_root = satellite_map_root
        
        # Load satellite map configuration
        if self.satellite_map_root is not None:
            f = open(os.path.join(self.satellite_map_root, 'map_prior_maptrv2.json'), 'r')
            content = f.read()
            self.satellite_map_config = json.loads(content)
        else:
            self.satellite_map_config = None
        ...
        Returns:
            dict: Data information that will be passed to the data \
                ...
                - lidar2img (list[np.ndarray], optional): Transformations \
                    from lidar to different cameras.
                - ann_info (dict): Annotation info.
                - satellite_map_path (str, optional): Satellite map path.
        """
        # Get base data info from parent class
        input_dict = super().get_data_info(index)
        
        # Add satellite map path if available
        if self.satellite_map_root is not None and self.satellite_map_config is not None:
            info = self.data_infos[index]
            input_dict['satellite_map_path'] = os.path.join(
                self.satellite_map_root, 
                self.satellite_map_config[info['token']]
            )
        else:
            input_dict['satellite_map_path'] = None
