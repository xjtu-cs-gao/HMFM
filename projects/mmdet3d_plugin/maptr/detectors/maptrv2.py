...
class MapTRv2(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.satellite_map_backbone = builder.build_backbone(satellite_map_backbone) if satellite_map_backbone is not None else None
    ...
    def forward_pts_train(self,
        ...
        outs = self.pts_bbox_head(
            pts_feats, lidar_feat, satellite_map_feat, img_metas, prev_bev)
    ...
    @force_fp32(apply_to=('img','points','prev_bev','satellite_map_img'))
    def forward_train(self,
        ...
        # print(satellite_map_img.shape, gt_seg_mask[0].shape)

        # # Debug visualization: overlay segmentation mask on satellite image and save
        # import matplotlib
        # import random
        # matplotlib.use('Agg')
        # import matplotlib.pyplot as plt
        # # image (keep original values/shapes)
        # img_t = satellite_map_img[0] if satellite_map_img.dim() == 4 else satellite_map_img
        # img_t = img_t.detach().cpu()
        # if img_t.dim() == 3 and img_t.shape[0] == 3:
        #     img_show = img_t.permute(1, 2, 0).numpy()
        #     img_kwargs = {}
        # elif img_t.dim() == 3 and img_t.shape[0] == 1:
        #     img_show = img_t[0].numpy()
        #     img_kwargs = {"cmap": "gray"}
        # else:
        #     img_show = img_t.numpy()
        #     img_kwargs = {}

        # # mask (keep original values/shapes)
        # mask_t = gt_seg_mask[0]
        # if isinstance(mask_t, list):
        #     mask_t = mask_t[0]
        # if mask_t.dim() == 3:
        #     mask_t = mask_t[0]
        # mask_t = mask_t.detach().cpu().numpy()
        # mask_kwargs = {"cmap": "gray"} if mask_t.ndim == 2 else {}

        # plt.figure(figsize=(6, 3))
        # plt.subplot(1, 2, 1)
        # plt.imshow(img_show, **img_kwargs)
        # plt.title('satellite')
        # plt.axis('off')
        # plt.subplot(1, 2, 2)
        # plt.imshow(mask_t, **mask_kwargs)
        # plt.title('mask')
        # plt.axis('off')
        # plt.tight_layout()
        # plt.savefig('/opt/data/private/maptr_sat/debug_vis_{}.png'.format(random.randint(0, 1000000)))
        # plt.close()

        satellite_map_feat = None
        if self.satellite_map_backbone is not None:
            satellite_map_feat = self.satellite_map_backbone(satellite_map_img)
            
        losses = dict()
        losses_pts = self.forward_pts_train(img_feats, lidar_feat, satellite_map_feat, gt_bboxes_3d,
    ...
    def forward_test(self, img_metas, img=None,points=None, satellite_map_img=None, **kwargs):
        ...
        new_prev_bev, bbox_results = self.simple_test(
            img_metas[0], img[0], points[0], satellite_map_img=satellite_map_img[0], prev_bev=self.prev_frame_info['prev_bev'], **kwargs)
    ...
    def pred2result(self, bboxes, scores, labels, pts, attrs=None):
        ...
        return result_dict
    def simple_test_pts(self, x, lidar_feat, satellite_map_feat, img_metas, prev_bev=None, rescale=False):
        """Test function"""
        outs = self.pts_bbox_head(x, lidar_feat, satellite_map_feat, img_metas, prev_bev=prev_bev)
    ...
    def simple_test(self, img_metas, img=None, points=None, satellite_map_img=None, prev_bev=None, rescale=False, **kwargs):
        ...
        new_prev_bev, bbox_pts = self.simple_test_pts(
            img_feats, lidar_feat, satellite_map_feat, img_metas, prev_bev, rescale=rescale)
