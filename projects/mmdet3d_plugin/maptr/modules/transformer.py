...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.satellite_map_fusion = builder.build_fusion_layer(satellite_map_fusion) if satellite_map_fusion is not None else None
    ...
    def forward(self,
        ...
        ouput_dic = self.get_bev_features(
            ...
            bev_pos=bev_pos,
            prev_bev=prev_bev,
            **kwargs)  # bev_embed shape: bs, bev_h*bev_w, embed_dims , (bs, 20000, 256)
        bev_embed = ouput_dic['bev']
        depth = ouput_dic['depth']
        if satellite_map_feat is not None and self.satellite_map_fusion is not None:
            bev_embed = self.satellite_map_fusion(bev_embed, satellite_map_feat)
        bs = mlvl_feats[0].size(0)
