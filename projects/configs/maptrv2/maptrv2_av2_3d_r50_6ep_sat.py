...
dataset_type = 'CustomAV2OfflineLocalMapDatasetWithSatelliteMap'
data_root = 'data/argoverse2/sensor/'
ann_root = 'annotation/'
satellite_map_root = 'data/av2_satellite'
file_client_args = dict(backend='disk')


train_pipeline = [
    dict(type='CustomLoadMultiViewImageFromFiles', to_float32=True, padding=True),
    dict(type='LoadandProcessSatelliteMapFromFile', to_float32=True, dataset='av2'),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=ann_root + 'av2_map_infos_train_filtered.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=ann_root + 'av2_map_infos_val_filtered.pkl',
        map_ann_file=ann_root + 'av2_gt_map_anns_val.json',
        satellite_map_root = satellite_map_root,
        # code_size=3,
        z_cfg=z_cfg,
...
optimizer_config = dict(type='GradientCumulativeFp16OptimizerHook', grad_clip=dict(max_norm=35, norm_type=2), cumulative_iters=2)
# learning policy
lr_config = dict(
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(type='WandbAccumulateLoggerHook',
             init_kwargs={
                 'project': 'MapTR_av2',  # 设置您的wandb项目名称
