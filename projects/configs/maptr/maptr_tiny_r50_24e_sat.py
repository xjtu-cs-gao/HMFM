...
model = dict(
    ...
    pretrain_maptr="ckpts/maptr_tiny_r50_24e.pth",
...
dataset_type = 'CustomNuScenesLocalMapDatasetWithSatelliteMap'
ann_root = 'annotation/'
data_root = 'data/nuscenes/'
satellite_map_root = 'data/satellite_map'
...
train_pipeline = [
    ...
    dict(type='LoadandProcessSatelliteMapFromFile', to_float32=True),
    ...
    dict(type='CustomCollect3D', keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'satellite_map_img'])
]
    ...
    dict(
        ...
        transforms=[
            ...
            dict(
                ...
                with_label=False),
            dict(type='CustomCollect3D', keys=['img', 'satellite_map_img'])
        ])
]
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=ann_root + 'nuscenes_infos_with_pose_temporal_train.pkl',
        satellite_map_root = satellite_map_root,
    ...
    val=dict(type=dataset_type,
             data_root=data_root,
             ann_file=ann_root + 'nuscenes_infos_with_pose_temporal_val.pkl',
...
optimizer_config = dict(type='GradientCumulativeFp16OptimizerHook', grad_clip=dict(max_norm=35, norm_type=2), cumulative_iters=4)
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
             init_kwargs={
                 'project': 'MapTR_Sat',  # 设置您的wandb项目名称
                 'name': 'satellite',         # 可选：设置本次运行的名称
                 # 其他wandb初始化参数
             })
    ])
